      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-brand-ivory z-50 shadow-2xl flex flex-col animate-slide-in-right">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-brand-beige">
          <div className="flex items-center gap-3">
            ...
            {count > 0 && (
              <span className="bg-brand-green text-white text-xs font-semibold px-2 py-0.5 rounded-full">{count}</span>
            )}
          </div>
          <button onClick={closeCart} className="p-2 rounded-full hover:bg-brand-beige transition-colors">
            <X size={20} />
          </button>
        </div>
              ...
              {cart.map((item) => (
                <div key={item.product.id} className="flex gap-4 bg-white rounded-2xl p-3 shadow-soft">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-brand-green font-semibold text-sm mt-1">{formatCRC(item.product.priceCRC * item.quantity)}</p>
          ...
          <div className="px-6 py-5 border-t border-brand-beige bg-white">
            ...
            <div className="flex gap-3">
              ...
              <Link
                href="/tienda"
                onClick={closeCart}
                className="flex-1 flex items-center justify-center gap-1.5 border-2 border-brand-green text-brand-green text-sm font-medium py-3 px-3 rounded-full hover:bg-brand-green/5 transition-all duration-200"
              >
                <ShoppingBag size={15} />
                Agregar más
              </Link>
              ...
              <Link
                href="/carrito"
                onClick={closeCart}
                className="flex-1 flex items-center justify-center gap-1.5 bg-brand-green text-white text-sm font-medium py-3 px-3 rounded-full hover:bg-brand-green/90 transition-all duration-200 group"
              >
                Finalizar pedido
